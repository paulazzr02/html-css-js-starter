...
/**
 * 자동 페이지 생성 스크립트
 * 사용법: node scripts/generate-page.js <페이지명> [옵션]
 */
...
if (!pageName) {
  console.error('사용법: node scripts/generate-page.js <페이지명> [제목] [설명] [옵션]');
  console.error('옵션:');
...
<html lang="${config.site.language}">
  ...
  <body>
    ...
    @@include('../includes/_script.html')
  </body>
</html>`;

// 파일 생성
const pagePath = path.join(__dirname, '..', 'src', 'html', `${pageName}.html`);
...
try {
  ...
  console.log(`\n📝 다음 단계:`);
  console.log(`1. src/html/${pageName}.html 파일을 편집하여 페이지 내용을 작성하세요`);
